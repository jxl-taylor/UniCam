package com.github.jelmerro.unicam;

import java.awt.BorderLayout;
import java.awt.Dimension;
import java.awt.Image;
import java.awt.Toolkit;
import java.awt.datatransfer.Clipboard;
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;
import java.awt.image.BufferedImage;
import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.util.Base64;
import java.util.Date;
import java.util.Timer;
import java.util.TimerTask;
import javax.imageio.ImageIO;
import javax.swing.JFrame;
import javax.swing.JOptionPane;

/**
 * The main frame of the application containing the WebcamPanel and the MenuBar
 *
 * @author Jelmerro
 */
public class Frame extends JFrame {

    private static Frame frame;
    private static ViewPanel panel;
    private static boolean fullscreen;
    private static boolean clicked;
    private static long timeClicked;
    private static Date pressedTime;
    private static MenuBar menuBar;

    /**
     * Since we only want one frame, the frame can only be accessed with this method
     *
     * @return frame Frame
     */
    public static Frame getInstance() {
        //If there is no frame yet, create it
        if (frame == null) {
            fullscreen = false;
            frame = new Frame("UniCam");
            //Listener for the closing of the window
            frame.addWindowListener(new WindowAdapter() {
                @Override
                public void windowClosing(WindowEvent e) {
                    System.exit(0);
                }
            });
            //Listener for mouse actions
            frame.addMouseListener(new MouseAdapter() {

                @Override
                public void mousePressed(MouseEvent e) {
                    //If there was recently clicked on the frame, detect the double click and toggle fullscreen
                    //Else set recently clicked to true and start a timer to set it to false again
                    if (clicked) {
                        frame.toggleFullscreen();
                        clicked = false;
                    } else {
                        clicked = true;
                        Timer t = new Timer(true);
                        t.schedule(new TimerTask() {

                            @Override
                            public void run() {
                                clicked = false;
                            }
                        }, 300);
                    }
                    //Remember the time of the click, to detect a mousebutton hold action
                    pressedTime = new Date();
                }

                @Override
                public void mouseReleased(MouseEvent e) {
                    //Sometimes after loading the pressedTime wasn't recorded
                    //Catching the nullpointer prevents this exception from being thrown
                    try {
                        //If the button was held long enough, start the copy to clipboard
                        timeClicked = new Date().getTime() - pressedTime.getTime();
                        if (timeClicked > 1000) {
                            Frame.getInstance().imageToClipboard();
                        }
                    } catch (NullPointerException ex) {

                    }
                }
            });
            //Set properties
            frame.setLayout(new BorderLayout());
            menuBar = new MenuBar();
            frame.setJMenuBar(menuBar);
            frame.pack();
            Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
            frame.setSize(new Dimension(screen.width / 2, (int) (screen.height / 1.5)));
            //Set the icon of the frame to the supplied base64 string
            String imageString = "";
            frame.setIconImage(frame.Base64ToImage(imageString));
        }
        //Always returns the frame
        return frame;
    }

    /**
     * Creates a new frame
     *
     * @param title String
     */
    private Frame(String title) {
        super(title);
    }

    /**
     * Sets the current panel in the frame
     *
     * @param p WebcamPanel
     */
    public void setPanel(ViewPanel p) {
        //Remove the current panel from the frame it exists
        if (panel != null) {
            frame.remove(panel);
        }
        //Switch to the supplied panel
        panel = p;
        add(panel);
        //Change the title to contain the current resolution
        setTitle("UniCam - " + (int) panel.getPreferredSize().getWidth() + "x" + (int) panel.getPreferredSize().getHeight());
    }

    /**
     * Getter for the current panel
     *
     * @return panel WebcamPanel
     */
    public ViewPanel getPanel() {
        return panel;
    }

    /**
     * Toggles between fullscreen and windowed mode
     */
    public void toggleFullscreen() {
        //Dispose the current frame
        dispose();
        //If going to fullscreen, don't draw a window
        setUndecorated(!fullscreen);
        //If going out of fullscreen, reset the frame to normal
        //Else maximize it
        if (fullscreen) {
            setExtendedState(JFrame.NORMAL);
            Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
            setSize(new Dimension(screen.width / 2, (int) (screen.height / 1.5)));
        } else {
            setExtendedState(JFrame.MAXIMIZED_BOTH);
        }
        setVisible(true);
        fullscreen = !fullscreen;
    }

    /**
     * Copies the current view of the panel to the clipboard
     */
    public void imageToClipboard() {
        try {
            BufferedImage bufferedImage = menuBar.getSettingsDialog().getCurrentWebcam().getImage();
            if (bufferedImage != null) {
                //Create a transferable image from the buffered image
                TransferableImage image = new TransferableImage(bufferedImage);
                //Copy the image to the clipbard
                Clipboard c = Toolkit.getDefaultToolkit().getSystemClipboard();
                c.setContents(image, null);
                //Display a mesage
                Frame.getInstance().getPanel().drawMessage("Copied " + new Date().toString().replace(":", ".") + " to clipboard");
            }
        } catch (NullPointerException ex) {
            //Nullpointers are to be expected and just mean that there is no webcam selected yet
        } catch (Exception ex) {
            //If another exception occurs, show an error
            JOptionPane.showMessageDialog(null, "Image could not be copied", "Clipboard failure", JOptionPane.ERROR_MESSAGE);
        }
    }

    /**
     * Converts a base64 String to an Image
     *
     * @param base64 String
     * @return image Image
     */
    public Image Base64ToImage(String base64) {
        try {
            byte[] imageBytes = Base64.getDecoder().decode(base64);
            Image image = ImageIO.read(new ByteArrayInputStream(imageBytes));
            return image;
        } catch (IOException ex) {
            return null;
        }
    }
}
